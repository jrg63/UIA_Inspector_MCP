...
class ScriptObj {
	...
	*/
	...
	/**
	 * Validates license with offline tolerance.
	 * Within ScriptObj.graceDays, skips online validation for faster startup.
	 * Set ScriptObj.graceDays before calling (0 = always validate online).
	 */
	static CheckOfflineLicense()
	{
		...
		if ScriptObj.graceDays > 0
		{
			last_check := RegRead(key, 'last_license_check', 0)
			if last_check && DateDiff(A_Now, last_check, 'Days') <= ScriptObj.graceDays
				return
		}
